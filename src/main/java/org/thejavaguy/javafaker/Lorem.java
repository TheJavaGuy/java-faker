...
public class Lorem {
  ...
  public String characters(int minimumLength, int maximumLength, boolean includeUppercase) {
    return characters(
        faker.random().nextInt(maximumLength - minimumLength) + minimumLength, includeUppercase);
  }

  public String characters(
      int minimumLength, int maximumLength, boolean includeUppercase, boolean includeDigit) {
    return characters(
  ...
  public String characters(
      int fixedNumberOfCharacters, boolean includeUppercase, boolean includeDigit) {
   ...
   * (wordCount+1)..(wordCount+randomWordsToAdd). Set {@code randomWordsToAdd} to 0 to generate
   * sentences with a fixed number of words.
   *
   * @param wordCount
  ...
  /**
   * Create a string with a fixed size. Can be useful for testing validator based on length string
   * for example
   *
