...
public class Internet {
  ...
  public String url() {
    return join(
        ...
        ".",
        FakerIDN.toASCII(
            faker.name().firstName().toLowerCase().replaceAll("'", "") + "-" + domainWord()),
        ".",
        domainSuffix());
  }

  /**
   * Generates a random avatar url based on a collection of profile pictures of real people. All
   * this avatar have been authorized by its awesome users to be used on live websites (not just
   * mockups). For more information, please visit: http://uifaces.com/authorized
   *
   * @return an url to a random avatar image.
   * @see <a href="http://uifaces.com/authorized">Authorized UI Faces</a>
   */
  public String avatar() {
    return "https://s3.amazonaws.com/uifaces/faces/twitter/"
        + faker.fakeValuesService().resolve("internet.avatar", this, faker);
  }

  /**
   * Generates a random image url based on the lorempixel service. All the images provided by this
   * service are released under the creative commons license (CC BY-SA). For more information,
   * please visit: http://lorempixel.com/
   *
  ...
  public String image() {
    String[] dimension =
        StringUtils.split(
            faker.fakeValuesService().resolve("internet.image_dimension", this, faker), 'x');
    if (dimension.length == 0) return "";
    return image(
        Integer.valueOf(StringUtils.trim(dimension[0])),
        Integer.valueOf(StringUtils.trim(dimension[1])),
        faker.bool().bool(),
        null);
  }
  ...
  public String image(Integer width, Integer height, Boolean gray, String text) {
    return String.format(
        "http://lorempixel.com/%s%s/%s/%s/%s",
        gray ? "g/" : StringUtils.EMPTY,
        width,
        height,
        faker.fakeValuesService().resolve("internet.image_category", this, faker),
  ...
  public String password(
      int minimumLength, int maximumLength, boolean includeUppercase, boolean includeSpecial) {
  ...
  public String password(
    ...
    if (includeSpecial) {
      ...
      for (int i = 0; i < faker.random().nextInt(minimumLength); i++) {
        password[faker.random().nextInt(password.length)] =
  ...
  public String macAddress(String prefix) {
    ...
    final int prefixLength = tmp.trim().isEmpty() ? 0 : tmp.split(":").length;
  ...
  public String privateIpV4Address() {
    ...
    final Integer[] PRIVATE_SECOND_OCTET_172 = {
