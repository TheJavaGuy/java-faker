...
public class NumberTest extends AbstractFakerTest {
  ...
  /**
   * Given a number of min/max ranges for each min/max range, call {@link Number#randomDouble(int,
   * int, int)} with min/max 'n' times calculate the uniqueness for that given min/max range. For
   * all 'uniqueness' values verify the percentage of 'uniqueness' ratios over 80% is 90%.
   *
   * <p>This isn't perfect but it ensures a pretty good degree of uniqueness in the random number
   * generation.
   */
  @Test
  public void randomDoubleRandomizationQuality() {
    Function<Pair<Long, Long>, Double> minMaxRangeToUniquePercentageFunction =
        minMax -> {
          final int min = minMax.getLeft().intValue(), max = minMax.getRight().intValue();
          long numbersToGet = calculateNumbersToGet(min, max);

          return uniquePercentageOfResults(
              numbersToGet, () -> faker.number().randomDouble(0, min, max));
        };

    final double percentGreaterThan80Percent =
        randomizationQualityTest(
            individualRunGtPercentUnique, minMaxRangeToUniquePercentageFunction);
    assertThat(percentGreaterThan80Percent)
        .as("Percentage of runs > 80% unique is gte 90%")
        .isGreaterThanOrEqualTo(percentRunsGtUniquePercentage);

    // this covers Issue # 121, the number of times the function is called with the MIN/MAX values
    // here
    // is RANDOMIZATION_TESTS_MAX_NUMBERS_TO_GET
    final double extremeRunUniquePercent =
        minMaxRangeToUniquePercentageFunction.apply(
            Pair.of((long) Integer.MIN_VALUE, (long) Integer.MAX_VALUE));
    assertThat(extremeRunUniquePercent)
        .as("Percentage of extreme runs > 80%")
        .isGreaterThanOrEqualTo(individualRunGtPercentUnique);
  }

  /**
   * Given a number of min/max ranges for each min/max range, call numberBetween with min/max 'n'
   * times calculate the uniqueness for that given min/max range. For all 'uniqueness' values verify
   * the percentage of 'uniqueness' ratios over 80% is 90%.
   *
   * <p>This isn't perfect but it ensures a pretty good degree of uniqueness in the random number
   * generation.
   */
  ...
  public void numberBetweenIntIntRandomizationQuality() {
    Function<Pair<Long, Long>, Double> minMaxRangeToUniquePercentageFunction =
        minMax -> {
          ...
          return uniquePercentageOfResults(
              numbersToGet, () -> faker.number().numberBetween(min, max));
        };
  ...
  /**
   * Given a number of min/max ranges for each min/max range, call {@link Number#numberBetween(long,
   * long)} with min/max 'n' times calculate the uniqueness for that given min/max range. For all
   * 'uniqueness' values verify the percentage of 'uniqueness' ratios over 80% is 90%.
   *
   * <p>This isn't perfect but it ensures a pretty good degree of uniqueness in the random number
   * generation.
   */
  ...
  public void numberBetweenLongLongRandomizationQuality() {
    ...
    // is RANDOMIZATION_TESTS_MAX_NUMBERS_TO_GET.
    final double extremeRunUniquePercent =
        minMaxRangeToUniquePercentageFunction.apply(Pair.of(Long.MIN_VALUE, Long.MAX_VALUE));
    assertThat(extremeRunUniquePercent)
        .as("Percentage of extreme runs > 80%")
        .isGreaterThanOrEqualTo(individualRunGtPercentUnique);
  }
   ...
   * RANDOMIZATION_QUALITY_RANGE_END, create a min/max range of -value/value and with of those
   * min/max values, call <em>percentUniqueRunner</em>.
   *
   * <p>Collect the number of calls to <em>percentUniqueRunner</em> that were above the threshold
   * and finally return that number divided by the total number of calls to
   * <em>percentUniqueRunner</em>.
   *
   * @return percent of percentUniqueRunner's results greater than the threshold
   */
  private double randomizationQualityTest(
      final double threshold, final Function<Pair<Long, Long>, Double> percentUniqueRunner) {
   ...
   * results, then calculates the number of results that were unique and returns the percentage that
   * where unique.
   */
