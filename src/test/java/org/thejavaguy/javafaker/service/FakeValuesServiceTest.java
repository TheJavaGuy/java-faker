...
public class FakeValuesServiceTest extends AbstractFakerTest {
  ...
  public void expressionWithInvalidFakerObject() {
    expressionShouldFailWith(
        "#{ObjectNotOnFaker.methodName}",
  ...
  public void expressionWithValidFakerObjectButInvalidMethod() {
    expressionShouldFailWith(
        "#{Name.nonExistentMethod}", "Unable to resolve #{Name.nonExistentMethod} directive.");
  }

  /**
   * Two things are important here: 1) the message in the exception should be USEFUL 2) a {@link
   * RuntimeException} should be thrown.
   *
   * <p>if the message changes, it's ok to update the test provided the two conditions above are
   * still true.
   */
  @Test
  public void expressionWithValidFakerObjectValidMethodInvalidArgs() {
    expressionShouldFailWith(
        "#{Number.number_between 'x','y'}",
  ...
  public void futureDateExpression() throws ParseException {
    ...
    Date date =
        dateFormat.parse(
            fakeValuesService.expression("#{date.future '10','TimeUnit.DAYS'}", faker));
  ...
  public void pastDateExpression() throws ParseException {
    ...
    Date date =
        dateFormat.parse(fakeValuesService.expression("#{date.past '5','TimeUnit.HOURS'}", faker));
